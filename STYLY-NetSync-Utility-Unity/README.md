...
| `org.nuget.r3` | 1.3.0 | R3 runtime assembly (`R3.dll`) |
...
> **Important:** If your project uses OpenXR (e.g. a Meta Quest build), add `USE_OPENXR` to **Project Settings → Player → Scripting Define Symbols**. Without it, a build with the OpenXR loader configured will fail with the message above.
>
> For an intentional non-XR build, removing the OpenXR loader from XR Plug-in Management is the correct resolution. Do **not** remove the loader from a project that is meant to ship as XR — that produces a player which hangs on the loading screen on device with no crash and no error.
